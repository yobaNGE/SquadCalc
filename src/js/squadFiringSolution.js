...
export default class SquadFiringSolution {
    ...
    constructor(weaponLatLng, targetLatLng, map, heightPadding) {
        ...
        this.gravity = App.activeWeapon.getGravity() * App.activeWeapon.gravityScale;
        ...
        this.trajectory = {low: null, high: null};
        this.elevation = {low: [], high: []};
        this.trajectory.low = this.getElevationSolution(this.distance, true);
        this.elevation.low.rad = this.trajectory.low.elevation;
        this.elevation.low.deg = this.radToDeg(this.elevation.low.rad);
        this.elevation.low.mil = this.radToMil(this.elevation.low.rad);
        this.trajectory.high = this.getElevationSolution(this.distance, false);
        this.elevation.high.rad = this.trajectory.high.elevation;
        ...
        this.timeOfFlight.low = this.getTimeOfFlight(this.elevation.low.rad, this.trajectory.low.heightDiff);
        this.timeOfFlight.high = this.getTimeOfFlight(this.elevation.high.rad, this.trajectory.high.heightDiff);
    ...
    */
    getElevation(dist = 0, lowangle = false) {
        return this.getElevationSolution(dist, lowangle).elevation;
    }

    getElevationSolution(dist = 0, lowangle = false) {
        if (App.activeWeapon.hasLaunchGeometry()) {
            return this.getElevationWithLaunchGeometry(dist, lowangle);
        }

        const heightDiff = this.heightDiff - App.activeWeapon.heightOffset;
        const elevation = this.solveDisplayElevation(dist, heightDiff, lowangle);

        return {
            elevation: this.validateElevation(elevation),
            distance: dist,
            heightDiff: this.heightDiff
        };
    }

    solveDisplayElevation(dist, heightDiff, lowangle = false) {
        const discriminant = this.velocity ** 4 - this.gravity * (this.gravity * dist ** 2 + 2 * heightDiff * this.velocity ** 2);

        if (dist <= 0 || discriminant < 0) {
            return NaN;
        }

        const angleFactor = lowangle ? -Math.sqrt(discriminant) : Math.sqrt(discriminant);
        return Math.atan((this.velocity ** 2 + angleFactor) / (this.gravity * dist)) - this.degToRad(App.activeWeapon.angleOffset);
    }

    validateElevation(elevation) {
        const elevationDeg = this.radToDeg(elevation);

        if (Number.isNaN(elevationDeg) || elevationDeg < App.activeWeapon.minElevation[0] || elevationDeg > App.activeWeapon.minElevation[1]){
            return NaN;
        }
    ...
    /**
     * Solves from the projectile spawn point while returning the displayed turret elevation.
     */
    getElevationWithLaunchGeometry(dist = 0, lowangle = false) {
        const launchOffset = App.activeWeapon.getLaunchOffset();
        ...
        let heightDiff = this.heightDiff - App.activeWeapon.heightOffset;
        let finalDist = baseDistance;
        let elevation = this.solveDisplayElevation(finalDist, heightDiff, lowangle);

        for (let i = 0; i < maxIterations; i++) {
            const physicalElevation = elevation + this.degToRad(App.activeWeapon.angleOffset);
            const offsetRotation = launchOffset.referenceElevationDeg === null
                ? physicalElevation
                : physicalElevation - this.degToRad(launchOffset.referenceElevationDeg);
            const sinElev = Math.sin(offsetRotation);
            const cosElev = Math.cos(offsetRotation);

            const muzzleVerticalOffset = launchOffset.forward * sinElev + launchOffset.vertical * cosElev;
            const muzzleHorizontalOffset = launchOffset.forward * cosElev - launchOffset.vertical * sinElev;

            finalDist = baseDistance - muzzleHorizontalOffset;
            heightDiff = this.heightDiff - App.activeWeapon.heightOffset - muzzleVerticalOffset;

            const newElevation = this.solveDisplayElevation(finalDist, heightDiff, lowangle);

            if (Number.isNaN(newElevation) || Math.abs(newElevation - elevation) < tolerance) {
    ...
    getBearing() {
        ...
        bearing += App.activeWeapon.displayYawOffsetDeg || 0;
     ...
     */
    getTimeOfFlight(angle, heightDiff = this.heightDiff){
        ...
        var t = this.velocity * Math.sin(angle) + Math.sqrt( (Math.pow(this.velocity, 2) * Math.pow(Math.sin(angle), 2)) + (2 * this.gravity * -heightDiff));
